...
$(function () {
    ...
    $('.alert-info').html(endpointInfoText);


    $.getJSON("getwebhookNotification", function (data) {
        console.log("Webhooks Notifications - ", data);
        var notifications = [];
        $.each(data, function (key, val) {
            var timestamp = new Date(val.timestamp).toISOString();
            notifications.push("<tr><td scope=\"row\">" + timestamp + "</td><td>" + val.orderId + "</td><td>" + val.transactionId + "</td><td>" + val.orderStatus + "</td><td>" + val.orderAmount + "</td></tr>");
        });
